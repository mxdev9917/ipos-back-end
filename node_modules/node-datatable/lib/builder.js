...
module.exports = function (options) {
  ...
  var self = {
    ...
    sCountTableName: options.sCountTableName,
    ...
    sGroupBySql: options.sGroupBySql,           // Custom caller SQL, added as AND where to add date range or other checks (caller must write the SQL)
  ...
  function buildCountStatement(requestQuery) {
    ...
    if(self.sCountTableName) {
      result += self.sCountTableName;
    }else{
      result += self.sFromSql ? self.sFromSql : self.sTableName;
    }
    result += buildWherePartial(requestQuery, false);
...
//            result += " WHERE (" + sWheres.join( ") AND (" ) + ")";
  ...
  function buildWherePartial(requestQuery) {
    var groupBy = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : true;

    var sGroupBySql = '';
    if(groupBy && self.sGroupBySql){
      sGroupBySql = self.sGroupBySql
    }
    var sWheres = [];
    var searchQuery = buildSearchPartial(requestQuery);
    if (searchQuery) sWheres.push(searchQuery);
    if (self.sWhereAndSql) sWheres.push(self.sWhereAndSql);
    var dateSql = buildDatePartial();
    if (dateSql) sWheres.push(dateSql);
    if (sWheres.length) return " WHERE (" + sWheres.join(") AND (") + ") " + sGroupBySql;
    return " " + sGroupBySql;
  }
